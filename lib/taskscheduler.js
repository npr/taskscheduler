...
TaskScheduler.wakeUpHandlers = function() {

    _.each(handlers, function(handlerID, handler) {
        // Reset suicide count
       handler.pollFailures = 0;
        queueNextJob(handler.jobFunction, handler.topic, handlerID);
    });
};
  ...
  taskPlugin.get(topic, function(err, message) {
    if (('undefined' != typeof message) && ('undefined' !== typeof message.body) && message.body) {
      jobFunction(topic, message.body, function cleanupCallback(err) {
        ...
        if (!err || handlers[handlerID].pollFailures >= workerSuicide) {
          message.del(function(err) {
            if (err) util.log("Error deleting a message: " + util.inspect(err));
            //util.log("DELETED: " + message.body);
          });
            // Start up a new job only if we are below suicide threshold
            if (handlers[handlerID] < workerSuicide) {
